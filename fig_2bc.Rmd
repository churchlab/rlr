...
```{r grouped_summarized}
current = binary_summ[rif == 12.5]

#current[color_cat == 1, color_set := 'gray44']
#current[color_cat == 2, color_set := 'gray44']
#current[color_cat == 3, color_set := 'gray44']
#current[color_cat == 4, color_set := 'gray44']


ggplot(current, aes(x=reorder(nickname,median_enrichment), y=median_enrichment, color = color_cat, fill = mut_class)) + 
...
```{r plot_concentration_change}
ggplot(binary_summ[mut_class=='Drug_resistance'], aes(x=rif, y=median_enrichment, group=nickname)) + 
  geom_errorbar(aes(ymin=min_enrichment, ymax=max_enrichment), color="black", alpha = 0.3, width=0.03) +  
  geom_point() + geom_line(position=position_dodge(width=0.03)) +
  scale_y_continuous(trans = log_trans(base=10),
                     breaks = c(.000001, .00001, .0001, .001, .01, .1, 1, 10, 100),
                     labels = log10(c(.000001, .00001, .0001, .001, .01, .1, 1, 10, 100))) +
