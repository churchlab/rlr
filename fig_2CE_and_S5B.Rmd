...
parent_folder <- here::here('fig_2CE_and_S5B')
...
ggsave(here::here('fig_2E_out.pdf'), plot=fig_2E,
...
```{r examine_turbidostat_data_more_closely}
figS5B <- ggplot(NULL)+
...
```
```{r save_1C}
ggsave(here::here('fig_2C_testagain.pdf'), plot=fig2C,
...
```{r save plot}
ggsave(here::here('fig_2C_out_test.pdf'), plot=fig_2C,
       width=2.5, height=3.3,
       units='in', dpi=300, device = cairo_pdf)
...
fig_2C <- ggplot(NULL)+
  #geom_line(data = edited_fractions_ev_tidy_annotated,
  #          aes(x = generations_annotated, y= fraction_edited,
  #               color = as.factor(ev_vial)), size = 1, alpha = 0.5) +
  geom_line(data = sim_data[r == 0.05],
            aes(x = generations, y= fraction_edited,
                 color = "black"), size = 1, linetype = "dashed") +
  geom_point(data = edited_fractions_ev_tidy_annotated,
             aes(x = generations_annotated, y= fraction_edited,
                 color = '#F8766D'), size =3, alpha = 0.7, stroke = 0) +
...
##alternative colors
```{r examine_turbidostat_data_with_generations}
color_series = c("3" = "#F53224", "7" = "#F75F55",
                 "11"= "#F98D85", "15", "$FCBBB6",
                 "sim" = "#000000")

sim_data$ev_vial = "sim"

fig_2C <- ggplot(NULL)+
  geom_line(data = sim_data[r == 0.05],
            aes(x = generations, y= fraction_edited,
                 color = as.factor(ev_vial)), size = 1, alpha = 0.7, linetype = "dashed") +
  geom_point(data = edited_fractions_ev_tidy_annotated,
             aes(x = generations_annotated, y= fraction_edited,
                 color = as.factor(ev_vial)), size =3, alpha = 0.7, stroke = 0) +
  
  theme_linedraw() +
  scale_y_continuous(breaks = c(0, .25, .5, .75, 1),
                     labels = c("0%","25%", "50%", "75%", "100%")) +
  scale_x_continuous(breaks = c(0, 20, 40, 60, 80, 100)) +
  scale_color_manual(values = color_series) +
  labs(
    y = "Fraction Edited",
    x = "Generations",
    color = "Replicate") +
