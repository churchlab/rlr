...
parent_folder = here::here('supplemental_figures/figS2')
...
fig_s2 = ggplot(h1, aes(x=more_desc, y=fraction_edited, color=more_desc)) + 
...
ggsave(here::here('supplemental_figures/fig_s2.pdf'),
...
fig_supp3 <- ggplot(h7, aes(x=genotype, y=fraction_edited, color=desc)) + 
  ...
  labs(
    x = "Genotype",
    y = "Fraction Edited",
    title = "Effect of genotype on editing efficiency, using PF020 SSAP",
...
y_breaks = c(.001, .01, .1, .25, .5, .9, 1)

fig_supp3B <- ggplot(h4_no_negative, aes(x=position, y=fraction_edited, color=as.factor(position))) + 
  ...
  theme(text = element_text(size=18)) +
  labs(
    x = "Heterology position within 90-mer donor segment",
    y = "Fraction Edited",
    color = "") 


ggsave(here::here('supplemental_figures/fig_s3B.pdf'), plot=fig_supp3B,
...
### throwing out this bottom section, H10 was repeated later


```{r h10}
h10 <- melt_with_fedit[expt == 'H10']
...
my_comparisons <- list( c("WT", "sbcB-"), c("sbcB-", "recJ-"),
                        c("recJ-","sbcB-, recJ-"), c("sbcB-, recJ-", "sbcB-, recJ-, mutS-") )
x_labels <- addline_format(c("WT", "∆sbcB", "∆recJ", "∆sbcB, ∆recJ", "∆sbcB, ∆recJ, ∆mutS"))

h10_2 = h10[time_point == 2]

ggplot(h10, aes(x=interaction(time_point, locus), y=fraction_edited, color=locus)) + 
  geom_jitter(width = 0.2, height = 0.05, size = 3) +
  theme_bw() +
  scale_y_continuous(breaks = c(.00001, .0001, .001, .01, .1, .25, .5, .9),
                     trans = log10_trans()) +
  theme(text = element_text(size=18)) +
  labs(
    x = "Locus targeted for TAG > TAA",
    y = "Fraction Edited",
    title = "PF020 editing across diverse loci") +
