...
parent_folder <- here::here('fig_4B_real')
...
fig4B <- ggplot(summary,
  ...
  geom_errorbar(aes(ymin=plas.enrichment-se, ymax=plas.enrichment+se),
    show.legend = FALSE, color='black', alpha=0.3, width=0.05) + #or width 0.1?
...
fig4B
```
#TODO: change figure height to be taller, thinner
```{r save_4B}
...
summary[, mut_class := factor(mut_class, levels=c('rpoB Alleles', 'Neutral',  'Lethal'))]
summary[, mut_class := factor(mut_class, levels=c('rpoB Alleles', 'Neutral',  'Lethal'))]
#only active timepoints for summary version
summary <- summary[timepoint %in% c(1,2,3)]
...
fig3b_summ <- ggplot(summary,
  ...
  scale_y_continuous(trans = log10_trans(),
                     ...
                     labels = c("0.3", "1", "3", "10", "100", "1,000", "10,000")) +
  scale_x_continuous(labels=condition_labels, breaks=summary[, unique(timepoint)], expand=c(0,0.3)) +
  labs(y='Allelic enrichment', x='Generations') +
  facet_grid(mut_class~., space='free_y', scales='free_y', switch='both') +
  annotation_logticks(sides = "l") +
  theme_linedraw() +
  geom_hline(yintercept=1, linetype=2, size=0.5) +
...
ggsave(here::here('fig_3b_summary.pdf'), plot=fig3b_summ, width=4, height=4, units='in', dpi=300, useDingbats=FALSE, device = cairo_pdf)
#save('megatable5', file=here::here('fig_3b','fig3bdata.RData'))
```
