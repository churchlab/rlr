...
fig2c <- ggplot(current_combined, aes(
...
color_vals_D = c('#F8766D', #red
                 '#B79F00') #gold

fig2d <- ggplot(binary_summ[mut_class=="Rifampicin Resistance" | nickname == 'rpoB_C_1576_G'], 
    aes(x=rif, y=mean_enrichment, group=nickname, color = mut_class )) + #color=log(median_enrichment) 
  #geom_errorbar(aes(ymin=median_enrichment - se_enrichment, ymax=median_enrichment + se_enrichment), color="black", alpha = 0.3, width=0.03) +  
  ...
  scale_colour_manual(values = color_vals_D[1:2]) +
  scale_fill_manual(values = color_vals_D[1:2]) +
