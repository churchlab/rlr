...
allelelistpath <- paste(parent_folder,"allele_annotated_list_UPDATE_2.csv", sep = "/" )
...
binary_summ <- into_summ[,
                        list(
                          ...
                          iqr_enrichment = IQR(enrichment.post.rif),
                          sd_freq = sd(seq_freq),
...
color_vals = c('grey44','#00BA38','#00BFC4','#F564E3','#619CCF', '#F8766D')

addline_format <- function(x,...){gsub(' ','\n',x)}


current = binary_summ[rif == 25] #rif concentration for binary plot here
...
fig2c <- ggplot(current, aes(x=reorder(nickname,median_enrichment), y=median_enrichment, color=as.factor(color_cat), fill = as.factor(color_cat))) + 
...
fig2c <- ggplot(current_combine_lethal, aes(
    ...
    y=pmax(1e-3, logmean_enrichment), 
    color= as.factor(color_cat),
    fill= as.factor(color_cat))) + 
  geom_linerange(aes(
    ymin=pmax(1e-3,logmean_enrichment-iqr_enrichment), 
    ymax=pmax(1e-3,logmean_enrichment+iqr_enrichment)),
    color="black", 
    alpha = 0.3) +  
  ...
  scale_colour_manual(values = color_vals[2:6]) +
  scale_fill_manual(values = color_vals[2:6]) +
  scale_y_continuous(
...
##version showing all data points:
```{r show all data}
before_after_1[, enrichment.post.rif := seq_freq / seq_freq[time_point==1],
      by=c("tx","nickname","rif")] #within each replicate, for each sequence

into_summ = before_after_1[time_point == 3]

current = into_summ[rif == 25]
current[, mut_class := factor(
  mut_class,
  levels=unique(mut_class)[c(1,2,3,4,6,5)],
  labels=addline_format(unique(mut_class)[c(1,2,3,4,6,5)]))]

fig2c <- ggplot(current, aes(x=reorder(nickname,enrichment.post.rif), y=enrichment.post.rif, color=as.factor(color_cat), fill = as.factor(color_cat))) + 
  #geom_errorbar(aes(ymin=min_enrichment, ymax=max_enrichment), color="black", alpha = 0.3) +  
  geom_point(size = 2) +
  scale_y_continuous(trans = log_trans(base=10),
                     breaks = c(.000001, .00001, .0001, .001, .01, .1, 1, 10, 100),
...
#prepare supplemental table 1B, list of figure 2CD results for rif alleles
```{r rif_allele_summary_supp}
output_table <- binary_summ[mut_class %in% c("Rifampicin Resistance",
                                             "rpoB Allele, unknown effect")]
output_table[, c("color_cat", "rpoB allele type", "mut_class",
...
fig2d <- ggplot(binary_summ[mut_class=="Rifampicin Resistance" | nickname == 'rpoB_C_1576_G'], 
    aes(x=rif, y=median_enrichment, group=nickname, color=log(median_enrichment))) + 
...
ggsave(here::here('fig2d.pdf'), plot=fig2d, width=5, height=3, units='in', dpi=300)
