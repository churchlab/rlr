...
depth_plot <- ggplot(dp_folA_region) + 
  ...
  geom_text(data=gene_pos, aes(x=start, y=gene_y_pos, 
    label=name), hjust=-0.3, vjust=-0.8, size=4) +
  scale_color_manual(values=gene_cols, guide=F) +
  new_scale_color() +
  labs( y = "Coverage Depth (relative to maximum coverage)",
       x = "Genome Position") +
  scale_x_continuous(limits = c(genome_start, genome_end), expand=c(0,0), 
                     breaks=genome_breaks, labels=genome_br_lbl) +
  scale_y_continuous(trans = log_trans(base=10), 
                    breaks = c(0.0001, 0.001, 0.01, 0.1, 1)) +
  coord_cartesian(xlim=c(genome_start, genome_end), ylim = c(1e-4,gene_y_pos*1.5), clip='on') +
  theme(
    strip.background=element_blank(), #strip.placement='outside',
    strip.text=element_text(color='black'), 
    panel.border=element_blank(),
  ...
  #add snp annotations
  annotate("text", x= 49765, y = 10^(0.1), label = "49765: C>T", hjust=0, angle=45, size=3) +
  annotate("text", x= 49903, y = 10^(-0.15), label = "49903: T>G", hjust=0, angle=45, size=3) +
  annotate("text", x= 50280, y = 10^(-0.3), label = "50280: T>G", hjust=0, angle=45, size=3)
...
col_plot <- ggplot(datatrim) + 
  ...
  scale_y_continuous(name='unique retron donors', breaks = c(1,2,3),
  ...
  scale_color_gradientn(
    name='reads/donor', colors=donor_cols, limits=c(1000,60000), 
...
combined_5c
ggsave('fig_5C_combined.pdf', plot=combined_5c, width=6, height=5, units='in', dpi=300, useDingbats = FALSE)
```
