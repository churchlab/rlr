...
annotated_fedit <- annotated_dt[ is_ref ==TRUE | is_alt == TRUE , fraction_edited := 1- (Count/sum(Count)) , by='uniq']
annotated_fedit <- annotated_fedit[,ref_alt_count := sum(Count), by = 'uniq'][is_ref == TRUE] #also trimmed table to one row per uniq
annotated_fedit <- annotated_fedit[,pct_ref_alt := ref_alt_count/all_count]


annotated_fedit

###DBG i also don't understand how fraction_edited could change based on if we are looking at ref or alt????
